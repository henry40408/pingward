...
Channels are editable (`/channels/{id}/edit` + `POST /channels/{id}`, the
`/admin/…` twins, and `PATCH /api/v1/channels/{id}`) under one rule: **a blank
submitted field keeps the stored value**, so a stored secret is never
...
(non-secret values + `has_*: bool` flags), which is what makes non-leakage a
type-level property. Webhook/Slack **URLs count as secrets** (they are
capability tokens); chat ids, ntfy server/topic and email recipients are
pre-filled. `ntfy_token_clear` is the one explicit-clear escape hatch, and
**`kind` is immutable** — `Store::update_channel` takes no `kind`. See
ARCHITECTURE.md's "Editing a channel without leaking its secrets".
Message content is `notify::event_text`, deliberately capped at **four lines**
— headline, `Project: x · every 5m (grace 1m)`, the reason, and a link to
`{PINGWARD_BASE_URL}/checks/{id}`. Everything past the headline comes from
`notify::EventDetail`, whose fields are all `Option` so a failed lookup drops a
line rather than the notification (`EventDetail::default()` renders the
original bare one-liner — that is the channel-test path). It is built **at the
call site from the pre-update check snapshot**, never re-read during delivery:
an `Up` event reports the ping *before* the recovery, which a re-read would
have already overwritten. `DownCause` is likewise the caller's to set —
